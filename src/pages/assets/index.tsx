...
export default function AssetsPage() {
    ...
    const filteredAssets = useMemo(() => {
        return assets.filter(asset => {
            const matchesSearch = asset.coinName.toLowerCase().includes(searchQuery.toLowerCase()) ||
                        ...
                        <div className="flex gap-2">
                            {filters.map((filter) => (
                                <Button
                                    ...
                                    className={`backdrop-blur-sm ${selectedFilter === filter.id
                                ...
                                <div className="relative overflow-hidden rounded-xl backdrop-blur-md bg-white/5 border border-white/10 p-6 shadow-2xl transition-colors hover:bg-white/[0.075]">
                                    <div className="flex items-center justify-between mb-4">
                                        <div className="flex items-center gap-3">
                                            <div className={`flex h-12 w-12 items-center justify-center rounded-full ${isProfit ? 'bg-green-500/10' : 'bg-red-500/10'
                                                }`}>
                                                <Icon
                                    ...
                                    <div className="space-y-3">
                                        ...
                                        <div className="flex justify-between text-sm">
                                            ...
                                            <span className={`font-medium ${isProfit ? 'text-green-500' : 'text-red-500'}`}>
                                                {isProfit ? '+' : ''}{formatCurrency(profitLoss)} ({profitLossPercentage.toFixed(2)}%)
                                            </span>
                                        </div>
                                    </div>
