...
export function PortfolioInsights({ data }: PortfolioInsightsProps) {
  return (
    <motion.div variants={fadeIn} className="grid grid-cols-1 md:grid-cols-3 gap-4">
      {/* Top Performers */}
      <Card className="backdrop-blur-md bg-card/50 border-border shadow-lg overflow-hidden">
          ...
          <div className="space-y-4">
            {data.topPerformers.map((performer, index) => (
              <motion.div
                ...
                className="relative overflow-hidden rounded-xl backdrop-blur-md bg-gradient-to-r from-blue-500/10 to-green-500/10 p-4 border border-border"
              >
                ...
                <div className="flex items-center justify-between relative z-10">
                  <div className="flex items-center gap-3">
                    ...
                    <span className="text-foreground font-medium">{performer.asset}</span>
                  </div>
            ...
            {data.riskExposure.map((category, index) => (
              ...
              >
                <div className="flex justify-between text-sm">
                  <span className="text-foreground/80">{category.category}</span>
                  <span className="text-foreground font-medium">{category.percentage}%</span>
                </div>
          ...
          <div className="space-y-4">
            {data.correlations.map((correlation, index) => (
              ...
              >
                <div className="absolute inset-0 bg-grid-white/5 [mask-image:linear-gradient(0deg,white,transparent)]" />
                <div className="flex items-center justify-between relative z-10">
                  <span className="text-foreground/80">{correlation.pair}</span>
                  <div className="flex items-center gap-2">
                    <div className="relative w-24 h-2 bg-foreground/10 rounded-full overflow-hidden backdrop-blur-sm">
                      <motion.div
                        initial={{ width: 0 }}
